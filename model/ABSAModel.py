...
def Predict(reviewData):
    ...
    dataDtm = vecSenNural.transform(data)
    dataDtm=hstack((dataDtm, extraFeatureDtm))
    YNuralPredict = svcNural.predict(dataDtm)
    ...
    for j in range(len(YPositivePredict)):
        ...
        indexNeutral=[]
        for i, (a, b) in enumerate(zip(predictAspect[j], YNuralPredict[j])):
            if a ==1 and b==1:
                indexNeutral.append(i) 
    
        r={}
        #3 positive 2 nural 1 negative
        if indexPositive:
            for index in indexPositive:
                r[sorted(mostCommonAspect)[index]]=3
        if indexNegative:
            for index in indexNegative:
                r[sorted(mostCommonAspect)[index]]=1
        if indexNeutral:
            for index in indexNeutral:
                r[sorted(mostCommonAspect)[index]]=2
        res.append(r)
